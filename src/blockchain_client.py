...
local chain is redeployed.

web3.py is imported lazily inside the functions that need it, so this
module and its pure helper functions can be imported and unit-tested
even in an environment where web3.py isn't installed.
"""
...
DEFAULT_DEPLOYMENT_PATH = os.path.join(
    "data",
    "output",
    "deployment.json",
)
...
DEFAULT_BLOCKCHAIN_RECORD_PATH = os.path.join(
    "data",
    "output",
...
class DeploymentInfoMissingError(BlockchainClientError):
    """Raised when deployment.json is missing."""


class ContractAbiMissingError(BlockchainClientError):
    """Raised when FingerprintRegistry.abi.json is missing."""


class BlockchainRecordMissingError(BlockchainClientError):
    """Raised when blockchain_record.json is missing."""


class RpcConnectionError(BlockchainClientError):
    """Raised when the local Hardhat node cannot be reached."""
...
def load_deployment_info(path=DEFAULT_DEPLOYMENT_PATH):
    ...
    if not isinstance(data, dict) or not data.get("contract_address"):
        raise DeploymentInfoMissingError(
            f"{path} does not contain a contract_address."
        )
...
def load_contract_abi(path=DEFAULT_ABI_PATH):
    ...
    if not isinstance(abi, list):
        raise ContractAbiMissingError(
            f"{path} does not contain a valid ABI list."
        )
...
def load_blockchain_record(path=DEFAULT_BLOCKCHAIN_RECORD_PATH):
    if not os.path.isfile(path):
        raise BlockchainRecordMissingError(
            ...
            "'python -m src.blockchain_upload' first."
        )
...
def _validate_sha256_hex(value):
    ...
    try:
        ...
        raise BlockchainClientError(
            f"Fingerprint hash is not valid hex: {value!r}"
        ) from exc


def hash_to_bytes32(hex_hash):
    """
    A SHA-256 hex digest is exactly 64 hex chars = 32 bytes,
    so it maps onto Solidity's bytes32 with no truncation or padding.
    """
...
def bytes32_to_hash(value):
    """
    Inverse of hash_to_bytes32.

    Accepts raw bytes or anything with a .hex() method
    such as HexBytes.
    """
...
def to_hex_string(value):
    """
    Format a transaction hash (HexBytes, bytes, or str)
    as a 0x-prefixed string.
    """
...
def get_default_account(w3):
    ...
    if not accounts:
        raise RpcConnectionError(
            "No accounts available from the local Hardhat node."
        )
...
def store_fingerprint_onchain(
    ...
    fingerprint_bytes = hash_to_bytes32(
        fingerprint_hash_hex
    )

    tx_hash = contract.functions.storeFingerprint(
        fingerprint_bytes,
        reference,
    ).transact(
        {
...
def get_record(contract, record_id):
    ...
    try:
        ...
        raise BlockchainClientError(
            f"Could not read blockchain record {record_id}: {exc}"
        ) from exc

    return {
        "record_id": record_id,
        "fingerprint_hash": bytes32_to_hash(
            fingerprint_bytes
        ),
        "reference": reference,
        "submitter": submitter,
...
def get_latest_record(contract):
    """
    Fetch the most recently stored record.
    """
    ...
    try:
        ...
        raise BlockchainClientError(
            f"Could not read the latest blockchain record: {exc}"
        ) from exc
